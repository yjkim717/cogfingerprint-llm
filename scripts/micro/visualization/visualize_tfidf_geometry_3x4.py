#!/usr/bin/env python3
"""
Visualize TF-IDF Geometry drift difference distribution in 3×4 panel format.

This script creates a single 3×4 panel plot for TF-IDF Geometry path_length feature:
- Rows: Academic / Blogs / News (domains)
- Columns: DS / G4B / G12B / LMK (models)
- Each cell: Drift difference for TF-IDF path_length feature (Human - LLM)
...
def get_tfidf_geometry_columns(df: pd.DataFrame) -> List[str]:
    """Get TF-IDF Geometry feature column names - only path_length."""
    return [
        col for col in df.columns
        if col.startswith("tfidf_") and col.endswith("_path_length")
    ]
...
def plot_tfidf_cell(differences: np.ndarray, ax) -> None:
    """Plot single cell for TF-IDF Geometry with optimized scaling for moderate ranges."""
    ...
    # TF-IDF Geometry has small ranges (typically 0.03-0.21 for most features, up to ~3 for tortuosity)
    ...
    # Note: The histogram shows only path_length feature differences (Human - LLM)
    # path_length has range typically 0.1-0.3, so we use fewer bins to make bars thicker
    if data_range > 2.0:
...
def plot_tfidf_geometry_3x4_panel(
    ...
    """Create 3×4 panel for TF-IDF Geometry with optimized scaling."""
    n_rows = len(domains)
    n_cols = len(models)
    ...
    for row_idx, domain in enumerate(domains):
        for col_idx, model in enumerate(models):
            ...
            plot_tfidf_cell(differences, ax)
    ...
    # Main title
    plt.suptitle(
        "TF-IDF Geometry Path Length Drift Difference Distribution",
        fontsize=14,
        fontweight="bold",
        y=0.995,
    )
    ...
    # Output filename
    output_path = output_dir / "tfidf_geometry_3x4_panel.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize TF-IDF Geometry drift difference distribution in 3×4 panel format"
    )
    parser.add_argument(
        "--domains",
        nargs="+",
    ...
    print("=" * 80)
    print("TF-IDF GEOMETRY DRIFT DIFFERENCE VISUALIZATION (3×4 Panel)")
    print("=" * 80)
    ...
    # Get TF-IDF Geometry path_length feature
    print("Processing TF-IDF Geometry path_length feature...")
    feature_cols = get_tfidf_geometry_columns(df)
    
    if not feature_cols:
        print("ERROR: No TF-IDF Geometry path_length feature found!")
        return
    ...
    print("=" * 80)
    print(f"Output saved to: {output_dir / 'tfidf_geometry_3x4_panel.png'}")
