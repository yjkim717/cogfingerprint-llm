#!/usr/bin/env python3
"""
CLI script for generating macro-level Academic texts using MacroAcademicGenerator.

This is a standalone script that can be run directly from the project root.
Usage:
    python generate_macro_academic_cli.py --model GEMMA_12B --levels 1
    python generate_macro_academic_cli.py --model DEEPSEEK --levels 1 2 3
"""
...
from generators import MacroAcademicGenerator


def main():
    """Main entry point for macro Academic generator CLI."""
    parser = argparse.ArgumentParser(
        description="Generate LLM texts for macro Academic dataset",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate Level 1 with DEEPSEEK
  python generate_macro_academic_cli.py --model DEEPSEEK --levels 1
  
  # Generate all levels with GEMMA_12B
  python generate_macro_academic_cli.py --model GEMMA_12B --levels 1 2 3
  
  # Generate Level 2 with GEMMA_4B
  python generate_macro_academic_cli.py --model GEMMA_4B --levels 2
  
  # Generate with LLAMA_MAVRICK
  python generate_macro_academic_cli.py --model LLAMA_MAVRICK --levels 1
  
  # Generate with 10 workers for faster processing
  python generate_macro_academic_cli.py --model GEMMA_12B --levels 1 --workers 10
        """
    )
    ...
    print()
    print("=" * 70)
    print("Starting macro Academic generation...")
    print("=" * 70)
    print()
    
    # Create generator
    generator = MacroAcademicGenerator(
