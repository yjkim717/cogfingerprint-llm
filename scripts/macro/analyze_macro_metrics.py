...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract Big Five and merged NELA features for macro datasets."
    )
    parser.add_argument(
        "--target",
        choices=["human", "llm"],
    ...
    parser.add_argument(
        "--skip-nela",
        action="store_true",
        help="Skip merged NELA extraction stage.",
    )
    parser.add_argument(
        "--skip-combine",
        action="store_true",
...
def main() -> None:
    ...
    if run_combine:
        ...
        if df_big5.empty or df_nela.empty:
            print("Skipping combination stage because one or both feature tables are empty.")
            return
        ...
        drop_meta_cols = [
            col
            for col in ["year", "item_index", "year_x", "year_y", "item_index_x", "item_index_y"]
